...
def training_main(args, model_cls, forward_step_function, create_dataset_function, handle_metrics_function=None,
                  init_function=None, collate_fn=None, forward_step_eval=None):
    ...
    if args.do_test and test_data is not None:
        ...
        test_loss = evaluate_and_print_results(prefix, iter(test_data),
                                               model, len(test_data) if args.strict_eval else args.eval_iters, args,
                                               timers, True, split='test', hooks=hooks)
...
def train(model, optimizer, lr_scheduler,
    ...
    while args.iteration < args.train_iters:
        ...
        def save_model(index):
            index = str(index)
            output_dir = os.path.join(args.save, f"{index}")
            if not os.path.exists(output_dir):
                os.makedirs(output_dir, exist_ok=True)
            state_dict = OrderedDict()
            for n, p in model.module.named_parameters():
                state_dict[n] = p.ds_tensor.detach().cpu()

            with open(os.path.join(output_dir, f'mp_rank_0{str(args.local_rank)}_model_states.pt'), 'wb') as f:
        ...
        if args.eval_interval and args.iteration % args.eval_interval == 0 and args.do_valid:
            ...
            evaluate_and_print_results(
                prefix, val_data_iterator, model, eval_iters, args, timers, False, step=args.iteration, split='val',
...
def train_step(data_iterator, model, optimizer, lr_scheduler,
    ...
    while True:
        ...
        if args.use_lomo:
            ...
            lr = lr_scheduler.get_lr()
            timers('backward').start()
            optimizer.fused_backward(lm_loss2, lr)
            timers('backward').stop()
        ...
        while iteration < eval_iters:
            ...
            for name in metrics:
                ...
                if not is_scalar[name] and is_last and metrics[name].shape[0] != last_shape[0]:
                    ...
                    metrics[name] = torch.concat([metrics[name],
                                                  torch.zeros([last_shape[0] - metrics[name].shape[0]] + shape[1:],
                                                              dtype=metrics[name].dtype, device=metrics[name].device)])
                if rank == 0:
                    metrics_gathered = [
                        torch.zeros_like(metrics[name], dtype=metrics[name].dtype, device=metrics[name].device) for _ in
