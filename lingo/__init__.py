...
import pynvml
import time
#from sat.model.official import *
import json
...
def let_lingo():
    ...
    print_stream('\033[0;33m[INFO] In conversations with Lingo, the language model will be trained automatically according to your needs, without requiring any effort on your part 😊\033[0m',0.005)
    print_stream('\033[0;33m[INFO] Would you like to command Lingo through casual conversation? \033[0m')

    user_conversation = input('\033[0;33m[INFO] If yes, please provide your OpenAI API KEY. If not, please type (No)\033[0m')
    print('')
...
def let_lingo_choice():
    print_stream('\033[0;33m[INFO] I see that you"re not too keen on chatting with me, but that"s alright. Let me ask you some questions next as part of the basic setup.\033[0m')
    print('')
    ...
    print('')
    model_choice = input('\033[0;36mPlease enter an option (number)\033[0m')
...
def let_lingo_conversation(ARGS):
    ...
    print_stream('\033[0;36m[AI] Hello there! I"m your AI assistant, and I"m here to help you train your model. Before we get started, it"s important to have a clear plan and goal in mind. \033[0m')
    ...
    while True:
        ...
        if result['choices'][0]['finish_reason'] == 'function_call':
            try:
                ...
                pass
            history.append({'role':"function",'name':'Set_ARGS','content':'ARGS["{}"] = {}\n It"s ok'.format(dict_json['Key'],dict_json['Value'])})
            data_input = None
        else:
            print_stream(
...
def get_cmd(ARGS):
    cmd = "deepspeed main.py --seed 1234"
    if ar
